...
async def classify_complaint(text: str) -> dict:
    """Классификация жалобы без автоматического расширения профиля клиники.

    Сначала проверяем утверждённые локальные признаки. Если жалоба не попала
    ни в профиль, ни в явный непрофиль, возвращаем can_help=None: ассистент
    обязан уточнить жалобу и не имеет права переходить к возрасту/записи.
    """
    ...
    return {
        "can_help": None,
        "service": local.get("service") or "не определено",
        "confidence": float(local.get("confidence") or 0),
        "reason": "Профиль жалобы не подтверждён: нужен один уточняющий вопрос.",
    }
...
- Если жалоба непрофильная (например, горло, зубы, кожа или изолированная боль в пятке) — не веди человека дальше по воронке записи.
- Если профиль жалобы неясен — задай один конкретный уточняющий вопрос. Не считай неизвестную жалобу профильной автоматически.
...
def _fallback_ack(text: str, lang: str = "ru") -> str:
    ...
    if lang == "kk":
        ...
        if any(w in low for w in ["өкше", "пятк", "пяточ", "шпор", "фасциит"]):
            return "Түсінемін 🙏🏻 Өкше ауыруы біздің клиниканың бекітілген бағытына жатпайды. Ортопед-травматологқа жүгінген дұрыс."
        if any(w in low for w in ["тамақ", "горло", "құлақ", "ухо", "тіс", "зуб"]):
            return "Клиниканың негізгі бағыты — арқа, буындар және неврология. Бұл шағым бойынша профильді дәрігерге жүгінген дұрыс 🌿"
        return "Нақтылап айтыңызшы, ауырсыну буында, арқада/мойында/белде ме, әлде басқа аймақта ма?"
    ...
    if any(w in low for w in ["пятк", "пяточ", "шпор", "подошв", "фасциит"]):
        return "Понимаю Вас 🙏🏻 С жалобами на боль в пятке наша клиника не ведёт приём. Лучше обратиться к ортопеду-травматологу."
    if any(w in low for w in ["горло", "ухо", "зуб", "стомат", "лор", "температур"]):
        return "Основной профиль клиники — позвоночник, суставы и неврология. По этому вопросу лучше обратиться к профильному врачу 🌿"
    return "Чтобы не подсказать неверно, уточните, пожалуйста: боль связана с суставом, спиной/шеей/поясницей или речь о другой области?"
