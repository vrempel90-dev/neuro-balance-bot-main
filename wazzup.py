...
def _is_outgoing_message(msg: dict[str, Any]) -> bool:
    """Определяет сообщения администратора/оператора, отправленные из Wazzup/CRM."""
    direction = str(
        _dig(msg, "direction")
        or _dig(msg, "message.direction")
        or _dig(msg, "status")
        or ""
    ).lower()
    return (
        ...
        or direction in {"out", "outgoing", "sent", "from_me"}
    )


def extract_dialog_messages(payload: dict[str, Any]) -> list[dict[str, str]]:
    """Достаёт входящие сообщения пациентов и исходящие сообщения админов для памяти GPT.

    Это не используется для отправки ответа. Только для обучения на живом стиле диалога:
    - role=user: пациент;
    - role=assistant: администратор/оператор.
    """
    raw_messages = payload.get("messages") or payload.get("message") or []
    if isinstance(raw_messages, dict):
        raw_messages = [raw_messages]
    ...
    for msg in raw_messages:
        ...
        if not text:
            continue
        role = "assistant" if _is_outgoing_message(msg) else "user"
        message_key = _message_id(msg, payload, chat_id, text)
        result.append({
            "chat_id": chat_id,
            "role": role,
            "text": text,
