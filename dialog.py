...
HARD_CONTRA_WORDS = [
    "кардиостимулятор", "дефибриллятор", "дефибрилятор",
    "инсулиновая помпа", "инсулиновой помпы", "помпа",
    "кохлеарный имплант", "кохлеарного импланта", "имплант",
    "металл", "метал", "металлоконструк",
    "беремен", "беременность", "жүктілік", "жукцилик",
    "онколог", "онкология", "рак", "эпилеп", "эпилепсия",
    "тромбоз", "свертываем", "свёртываем",
    "декомпенсированн", "диабет", "тиреотоксикоз",
...
def _answer_has_greeting(answer: str) -> bool:
    low = _low(answer)
    return "здравствуйте" in low or "сәлеметсіз" in low or "салеметсіз" in low


def _already_greeted(session: dict[str, Any]) -> bool:
    if session.get("greeted") is True:
        return True

    last_answer = str(session.get("last_assistant_answer") or session.get("last_bot_answer") or "")
    if _answer_has_greeting(last_answer):
        return True

    return session.get("step") not in (None, "", "start")
...
def _finalize(chat_id: str, session: dict[str, Any], answer: str) -> str:
    ...
    if _answer_has_greeting(answer):
...
def _ask_complaint(session: dict[str, Any]) -> str:
    if _already_greeted(session):
        return _tr(
            session,
            "Подскажите, пожалуйста, что Вас беспокоит? Я сориентирую, относится ли это к профилю нашей клиники 🌿",
            "Нақты не мазалайды? Бұл біздің клиниканың бағытына жата ма — соны айтып, бағыттаймын 🌿",
        )
...
def _profile_greeting_prefix(session: dict[str, Any]) -> str:
    return "" if _already_greeted(session) else "Здравствуйте! "


def _profile_age_answer_ru(session: dict[str, Any], text: str = "") -> str:
    low = _low(text)
    prefix = _profile_greeting_prefix(session)

    if _has_leg_radiation_profile(text):
        return f"{prefix}Да, боль в пояснице, которая отдаёт в ногу, относится к нашему профилю 🌿 Подскажите, пожалуйста, сколько Вам лет?"

    if any(w in low for w in ["протруз", "грыж"]):
        return f"{prefix}Да, боль в спине, протрузии и грыжи относятся к нашему профилю 🌿 Подскажите, пожалуйста, сколько Вам лет?"

    if any(w in low for w in ["поясниц", "пояснич"]):
        return f"{prefix}Да, боль в поясничной области относится к нашему профилю 🌿 Подскажите, пожалуйста, сколько Вам лет?"

    if any(w in low for w in ["спина", "спине", "спину", "спины"]):
        return f"{prefix}Да, боль в спине относится к нашему профилю 🌿 Подскажите, пожалуйста, сколько Вам лет?"

    return f"{prefix}Да, это относится к нашему профилю 🌿 Подскажите, пожалуйста, сколько Вам лет?"


def _profile_confirm_ru(session: dict[str, Any]) -> str:
    prefix = _profile_greeting_prefix(session)
    return f"{prefix}Да, это относится к нашему профилю 🌿"
...
def _profile_confirm_and_ask_age(session: dict[str, Any]) -> str:
    return _tr(
        session,
        _profile_age_answer_ru(session),
...
def _ask_age_contextual(session: dict[str, Any], text: str) -> str:
    ...
    parts_ru.append(_profile_age_answer_ru(session, text))
...
def _ask_contra(session: dict[str, Any]) -> str:
    return _tr(
        session,
        "Спасибо 🌿 Перед записью уточню противопоказания для безопасности. Нет ли у Вас: кардиостимулятора/дефибриллятора, инсулиновой помпы, кохлеарного импланта, беременности, онкологии, металла в зоне лечения, эпилепсии, тромбоза или нарушения свёртываемости, высокой температуры/острой инфекции, декомпенсированного диабета или тиреотоксикоза? Также приём не проводится младше 16 или старше 75 лет, а также при ограниченной подвижности — коляска, костыли. Противопоказаний нет?",
        "Жақсы 🌿 Жазылу алдында қауіпсіздік үшін қарсы көрсетілімдерді нақтылаймын. Сізде: кардиостимулятор/дефибриллятор, инсулин помпасы, кохлеарлық имплант, жүктілік, онкология, емдеу аймағында металл, эпилепсия, тромбоз/қан ұю бұзылысы, қызу/жедел инфекция, декомпенсацияланған диабет немесе тиреотоксикоз жоқ па? Сондай-ақ 16 жасқа дейінгі, 75 жастан асқан және қозғалысы шектеулі пациенттерге қабылдау жүргізілмейді. Қарсы көрсетілімдер жоқ па?",
    )
