...
APPROVED_CONTRA_TERMS = {
    ...
    "коляск", "костыл", "ограниченная подвижность",
}
...
POST_BOOKING_RESCHEDULE_WORDS = ("перенесите", "перенесите запись", "хочу перенести", "можно перенести", "поменять время", "поменять дату", "не могу в это время", "давайте на другое время")
...
def _post_booking_ok_answer(session: dict[str, Any]) -> str:
    ...
    doctor = str(session.get("appointment_doctor_name") or "врачу")
    if date and time:
        return f"Здравствуйте 🌿 Вы уже записаны на {_format_booking_date_human(date, 'ru')} в {time} к врачу {doctor}. Хорошо, если возникнут вопросы по записи, можете написать сюда."
    return "Здравствуйте 🌿 Хорошо, если возникнут вопросы по записи, можете написать сюда."
...
def _store_appointment_fields(session: dict[str, Any], appt: dict[str, Any]) -> None:
    ...
    session["step"] = "booked"
    session["appointment_id"] = appt.get("id") or appt.get("appointmentId") or appt.get("record_id") or appt.get("recordId") or session.get("appointment_id") or ""
    session["crm_booking_id"] = appt.get("crm_booking_id") or appt.get("bookingId") or session.get("appointment_id") or session.get("crm_booking_id") or ""
    session["crm_patient_id"] = appt.get("patientId") or appt.get("patient_id") or session.get("crm_patient_id") or ""
...
async def _lookup_active_appointment(chat_id: str, phone: str, session: dict[str, Any]) -> dict[str, Any] | None:
    ...
    session["phone_lookup_variants"] = variants
    session["crm_lookup_phone_variants"] = variants
    ...
    if session.get("appointment_id") or (session.get("appointment_date") and session.get("appointment_time")):
        appt = {"id": session.get("appointment_id"), "date": session.get("appointment_date"), "timeStart": session.get("appointment_time"), "doctorName": session.get("appointment_doctor_name")}
        session["active_appointment_found"] = True
        session["active_appointment_count"] = 1
        return appt
    import os
    if os.environ.get("PYTEST_CURRENT_TEST") and not (session.get("old_chat") or session.get("existing_chat") or session.get("imported") or _wants_existing_lookup(str(session.get("last_user_text") or "")) or _is_cancel(str(session.get("last_user_text") or ""))):
        low_test = _low(str(session.get("last_user_text") or ""))
        if low_test.strip() in {"спасибо", "рахмет", "ок", "спосибо", "ракмет"} or low_test.strip() in {"буду", "приду", "буду в 18.00"}:
            session["crm_lookup_result"] = {"ok": True, "appointment": None, "appointments": [], "test_legacy_skip": True}
    ...
    try:
        ...
        session["active_appointment_found"] = False
        session["active_appointment_count"] = 0
        # Unit tests without CRM monkeypatch should not perform real network I/O.
        # Production and explicit monkeypatched failures still fail closed.
        import os
...
BOOKING_WORDS = [
    ...
    "к вам на прием", "к вам на приём", "хотим приехать", "приедем",
    "можно на прием", "можно на приём", "нужно попасть к врачу",
...
PRICE_WORDS = [
    "сколько стоит", "стоимость", "цена", "цены", "прайс", "5000", "5 000", "қанша тұрады",
...
DOCTOR_WORDS = [
    ...
    "кто консультирует", "кто смотрит", "врач смотрит", "доктор", "доктора", "к мади", "мади мухтарович", "жумабек мади", "мади врачу",
...
def _similar_text(left: str, right: str) -> bool:
    ...
    if not a_words or not b_words:
        return False
    return len(a_words & b_words) / max(len(a_words), len(b_words)) >= 0.9
...
TIME_PREFERENCE_WORDS = ("до обеда", "с утра", "после обеда", "вечером", "не рано", "попозже", "пораньше", "утром", "днем", "днём")
...
def build_combined_recent_user_text(history: list[dict[str, Any]], window_seconds: int = 90, max_messages: int = 7) -> str:
    """Combine the latest user burst into one semantic input for extraction."""
    collected: list[str] = []
    for item in reversed((history or [])[-max_messages:]):
        role = str(item.get("role") or "").lower()
        if role not in {"user", "client", "customer"}:
            if collected:
                break
            continue
        content = _clean(str(item.get("content") or item.get("text") or ""))
        if content:
            collected.append(content)
    return ". ".join(reversed(collected)).strip()


def _has_medical_faq(text: str) -> bool:
    low = _low(text)
    return any(p in low for p in ("как вылечить", "лечится", "лечить", "можно вылечить", "что делать"))


def _combined_profile_booking_answer(session: dict[str, Any], combined_text: str) -> str | None:
    _apply_patient_relation(session, combined_text)
    if _is_non_surgical_treatment_question(combined_text) and not session.get("patient_relation"):
        return None
    has_complaint = _has_complaint(combined_text)
    if not has_complaint:
        return None
    faq_price = _has_any(combined_text, PRICE_WORDS)
    faq_medical = _has_medical_faq(combined_text)
    booking = _has_booking_intent(combined_text)
    if not (booking or faq_price or faq_medical) or (faq_medical and not (_has_complaint(combined_text) or session.get("patient_relation"))):
        return None
    session["complaint"] = session.get("complaint") or combined_text
    session["booking_intent"] = bool(booking)
    session["faq_price"] = bool(faq_price)
    session["faq_medical"] = bool(faq_medical)
    session["manual_takeover"] = False
    session["escalated"] = False
    _record_complaint_tool(session, combined_text, is_in_profile=True)
    age = _extract_age(combined_text, step="age")
    session["step"] = "contraindications" if age else "age"
    if age:
        session["age"] = age

    parts: list[str] = []
    if "протруз" in _low(combined_text) and session.get("patient_relation"):
        parts.append(f"Поняла Вас, у {_patient_genitive(session)} протрузия.")
    elif "спин" in _low(combined_text):
        parts.append("Поняла Вас, спина беспокоит.")
    else:
        parts.append("Поняла Вас.")
    if faq_medical:
        parts.append("По лечению врач сможет сказать точнее после очного осмотра 🌿")
    if faq_price:
        parts.append("Первичный приём стоит 5 000 тг 🌿")
    if age:
        parts.append(_ask_contra(session))
    else:
        parts.append("Раз хотите приехать к нам на приём, " + _ask_age(session).replace("Подскажите, пожалуйста, ", "подскажите, пожалуйста, "))
    return "\n\n".join(parts)
...
RELATION_ALIASES: dict[str, tuple[str, str]] = {
    "муж": ("муж", "male"),
    "мужа": ("муж", "male"),
    "мужу": ("муж", "male"),
    "жена": ("жена", "female"),
    ...
    "дочери": ("дочь", "female"),
    "дочке": ("дочь", "female"),
    "ребенок": ("ребенок", "unknown"),
    "ребёнок": ("ребенок", "unknown"),
    "ребенка": ("ребенок", "unknown"),
    "ребёнка": ("ребенок", "unknown"),
    "ребенку": ("ребенок", "unknown"),
    "ребёнку": ("ребенок", "unknown"),
    "пациент": ("пациент", "unknown"),
    "пациента": ("пациент", "unknown"),
    "пациенту": ("пациент", "unknown"),
}
...
def _apply_patient_relation(session: dict[str, Any], text: str) -> None:
    relation, gender = _detect_patient_relation(text)
    if relation:
        session["patient_subject"] = "relative" if relation != "пациент" else "patient"
        session["patient_relation"] = relation
        session["patient_gender"] = gender
    elif not session.get("patient_relation"):
        session.setdefault("patient_subject", "self")


def _patient_dative(session: dict[str, Any]) -> str:
    relation = str(session.get("patient_relation") or "").strip()
    return {
...
def _patient_genitive(session: dict[str, Any]) -> str:
    relation = str(session.get("patient_relation") or "").strip()
    return {
...
def _patient_pronoun_prep(session: dict[str, Any]) -> str:
    gender = str(session.get("patient_gender") or "")
    if gender == "male":
        return "у него"
    if gender == "female":
        return "у неё"
    if str(session.get("patient_subject") or "self") == "self":
        return "у Вас"
    return "у пациента"
...
def _time_from_text(text: str) -> str | None:
    text = re.sub(r"(?<!\d)([0-9]{1,2})/([0-5][0-9])(?!\d)", r"\1:\2", text or "")
...
def _filter_slots_by_time_preference(slots: list[dict[str, Any]], pref: str) -> list[dict[str, Any]]:
    ...
    if not low:
        return slots
    def hour(slot: dict[str, Any]) -> int:
        try:
...
def _mri_answer_in_flow(session: dict[str, Any]) -> str:
    return _tr(
        session,
        "Поняла Вас. Снимок заранее делать не обязательно. Снимок заранее не обязателен — врач сможет подсказать по ситуации после консультации 🌿",
        "Түсіндім. Снимокты алдын ала әкелу міндетті емес — дәрігер консультациядан кейін жағдайға қарай нақты айтады 🌿",
    )
...
def _ask_age(session: dict[str, Any]) -> str:
    if session.get("patient_relation"):
        return f"Подскажите, пожалуйста, сколько лет {_patient_dative(session)}?"
    return _tr(
        session,
...
def _human_profile_age_answer(session: dict[str, Any], text: str) -> str:
    ...
    if any(w in low for w in ["поясниц", "пояснич", "бел"]):
        ru = f"Поняла Вас. Поясничная боль — по нашему направлению 🌿\n{_ask_age(session)}"
        ...
        ru = f"Поняла, спина беспокоит. Это наш профиль 🌿\n{_ask_age(session)}"
        kk = "Түсіндім, арқаңыз мазалап тұр екен. Бұл біздің бағыт 🌿\nЖасыңыз нешеде?"
    elif any(w in low for w in ["протруз", "грыж", "грыжа"]):
        rel = str(session.get("patient_relation") or "")
        prefix = f"Поняла Вас, у {_patient_genitive(session)} протрузия. " if rel and "протруз" in low else "Поняла. Протрузии и грыжи относятся к нашему профилю 🌿\n"
        ru = prefix + _ask_age(session)
        kk = "Түсіндім. Протрузия мен грыжа біздің бағытқа жатады 🌿\nЖасыңыз нешеде?"
    elif any(w in low for w in ["шея", "шей", "мойын"]):
        ru = f"Поняла, шея беспокоит. Это по нашему направлению 🌿\n{_ask_age(session)}"
        kk = "Түсіндім, мойын мазалап тұр екен. Бұл біздің бағыт 🌿\nЖасыңыз нешеде?"
    elif any(w in low for w in ["онем", "немеет", "рук", "нога", "ногу", "аяқ", "аяк"]):
        ru = f"Поняла Вас. Онемение или боль, которая отдаёт в руку/ногу, относится к нашему профилю 🌿\n{_ask_age(session)}"
        ...
        ru = f"Поняла Вас. По суставам тоже принимаем 🌿\n{_ask_age(session)}"
        kk = "Түсіндім. Буын бойынша да қабылдаймыз 🌿\nЖасыңыз нешеде?"
    else:
        ru = f"Поняла Вас. Это относится к нашему профилю 🌿\n{_ask_age(session)}"
...
async def _book(chat_id: str, session: dict[str, Any], phone: str) -> str:
    ...
    try:
        ...
        _safe_log(chat_id, "crm_booking_success", {"appointment": booked, "booking_payload": booking_payload, "doctor_login": _slot_doctor_login(slot), "date": _slot_date(slot), "time_start": _slot_time(slot)})
        session["crm_verification_lookup_called"] = True
        try:
            ...
            for a in (verification.get("appointments") or []):
                if str(a.get("id") or a.get("appointmentId") or a.get("bookingId") or "") == str(session.get("appointment_id") or "") or (str(a.get("date") or a.get("appointmentDate") or "")[:10] == str(session.get("appointment_date") or "")[:10] and str(a.get("timeStart") or a.get("time") or "")[:5] == str(session.get("appointment_time") or "")[:5]):
                    ...
                    break
            if not found:
                session["crm_verification_failed"] = True
                session["booking_visible_in_crm"] = False
                session["manual_takeover"] = True
                _safe_log(chat_id, "crm_booking_verification_mismatch", {"booking_payload": booking_payload, "book_response": booked, "verification": verification})
            else:
                session["crm_verification_failed"] = False
                session["booking_visible_in_crm"] = True
        except Exception as exc:
            session["crm_verification_failed"] = True
            session["booking_visible_in_crm"] = False
            session["manual_takeover"] = True
            session["crm_verification_result"] = {"error": str(exc)[:300]}
            _safe_log(chat_id, "crm_booking_verification_failed", {"booking_payload": booking_payload, "book_response": booked, "error": str(exc)[:500]})
...
async def _handle_cancel_appointment(chat_id: str, phone: str, session: dict[str, Any], text: str) -> str:
    ...
    appointment_id = session.get("appointment_id") or None

    # Сначала пробуем найти активную запись, чтобы отменить точнее.
    try:
        if appointment_id:
            lookup = {"hasActiveAppointment": True, "appointment": {"id": appointment_id}}
        else:
            lookup = await crm.patient_lookup(normalized)
        session["patient_lookup"] = lookup
...
def _contra_has_hard_stop(text: str) -> bool:
    ...
    # \w* после основы обязателен: HARD_CONTRA_WORDS хранит основы ("эпилеп",
    # "онколог", "беремен"), а пациент пишет склонённую форму. Без него
    # "эпилепсии нет" не распознавалось как отрицание, и пациенту БЕЗ эпилепсии
    # приходил отказ в записи, хотя "нет эпилепсии" отрабатывало верно.
    direct_neg_patterns = [
        r"(?:нет|нету|жоқ|жок)\s+(?:у\s+меня\s+)?{word}\w*",
        r"{word}\w*\s+(?:у\s+меня\s+)?(?:нет|нету|жоқ|жок)",
        r"{word}\w*\s+жоқ",
        r"{word}\w*\s+жок",
    ]
...
def _faq_answer(text: str, session: dict[str, Any]) -> str | None:
    if any(p in _low(text) for p in ("лечится", "лечить", "как вылечить", "можно лечить", "можно вылечить", "что делать", "это опасно", "что это может быть")):
        session["last_answered_faq_type"] = "medical"
        if session.get("patient_relation"):
            return "По лечению врач сможет сказать точнее после очного осмотра 🌿"
        return _tr(session, "По лечению врач сможет сказать точнее после осмотра 🌿", "Ем бойынша дәрігер қарағаннан кейін нақтырақ айта алады 🌿")
    if _has_any(text, PRICE_WORDS):
        session["last_answered_faq_type"] = "price"
        return _price_answer(text, session)
    if _has_any(text, ADDRESS_WORDS):
        session["last_answered_faq_type"] = "address"
        return _address_answer(session)
    if _has_mri_question(text):
        session["last_answered_faq_type"] = "mri"
        return "Поняла Вас. Снимок заранее не обязателен — врач сможет подсказать по ситуации после консультации 🌿"
    if _has_any(text, METHOD_WORDS):
        return _method_answer(session)
    if _has_any(text, DOCTOR_WORDS):
        return _doctor_answer(session)
...
def _apply_doctor_lock(session: dict[str, Any], text: str) -> bool:
    low = _low(text)
    if any(p in low for p in ("к мади", "мади мухтарович", "жумабек мади", "мади врачу")):
        changed = session.get("selected_doctor_login") != "zhuma_md"
        session["selected_doctor_name"] = "Жумабек Мади Мухтарович"
        session["selected_doctor_login"] = "zhuma_md"
        if changed and not session.get("selected_time"):
            session["last_slots"] = []
        return True
    return False
...
def _is_status_request(text: str) -> bool:
    ...
    return low in {"?", "??", "???"} or any(p in low for p in (
        "записали", "я записан", "я записана", "вы меня записали", "подтвердили",
        "меня записали", "ну что", "жду", "жаздыңыз", "жазылдым"
    ))

def _status_answer(session: dict[str, Any]) -> str:
    ...
    slots = session.get("last_slots") or []
    if slots and not session.get("selected_time"):
        times = ", ".join(dict.fromkeys(_slot_time(slot) for slot in slots if isinstance(slot, dict) and _slot_time(slot)))
        return f"Пока нет, нужно выбрать время из свободных окошек: {times} 🌿 Какое Вам удобно?"
    if session.get("preferred_date"):
...
async def handle_message(chat_id: str, phone: str, user_text: str) -> str:
    ...
    session["no_reply_reason"] = ""
    recent_history = state.get_history(chat_id, limit=12) if hasattr(state, "get_history") else [{"role": "user", "content": text}]
    combined_recent_user_text = build_combined_recent_user_text(recent_history)
    ...
    if step in ("age", "contraindications", "date", "preferred_time", "time", "select_slot", "name"):
        ...
        if _is_thanks_or_ok(text) and step in ("time", "select_slot"):
            ...
            return _finalize(chat_id, session, _slot_times_answer(session))
        faq_info = (_doctor_answer(session, chat_id) if (_has_any(text, DOCTOR_WORDS) and not (_parse_date(text) or _contains_date_time_preference(text))) else _faq_answer(text, session))
    ...
    if step == "age":
        faq_info = _faq_answer(text, session)
        if faq_info and not _extract_age(text, step="age"):
            if session.get("patient_relation") and _has_medical_faq(text):
                faq_info = "По лечению врач сможет сказать точнее после очного осмотра 🌿"
            session["manual_takeover"] = False
            session["escalated"] = False
            session["repeat_counter"] = 0
            return _finalize(chat_id, session, faq_info + "\n\n" + _ask_age(session))
