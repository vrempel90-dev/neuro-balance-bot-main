...
def _session_has_dialog_context(session: dict[str, Any]) -> bool:
    step = session.get("step") or "start"
    return bool(
        step not in ("start", "", None)
        or session.get("complaint")
        or session.get("profile_status")
        or session.get("age")
...
def _load_session_for_identity(chat_id: str, phone: str) -> dict[str, Any]:
    session = state.get_session(chat_id)
    if not isinstance(session, dict):
        session = {}

    # Wazzup chat_id can vary between payload shapes.  If this chat_id has no
    # meaningful context yet, recover the existing dialog by stable phone.
    allow_phone_fallback = (
        session.get("source") == "wazzup"
        or str(chat_id or "") == str(phone or "")
        or str(phone or "") in str(chat_id or "")
        or "wazzup" in str(chat_id or "").lower()
    )
    if phone and allow_phone_fallback and not _session_has_dialog_context(session):
        try:
            by_phone = state.find_session_by_phone(phone)
        except Exception:
            by_phone = None
        if (
            isinstance(by_phone, dict)
            and _session_has_dialog_context(by_phone)
            and (by_phone.get("source") == "wazzup" or str(chat_id or "") == str(phone or ""))
        ):
            by_phone["session_recovered_by_phone"] = True
            by_phone["last_chat_id"] = chat_id
            return by_phone
...
def _has_generic_lead_prompt(text: str) -> bool:
    low = _low(text)
    return any(marker in low for marker in GENERIC_LEAD_PROMPT_MARKERS)
...
def _ask_age_contextual(session: dict[str, Any], text: str) -> str:
    ...
    elif _has_back_protrusion_profile(text):
        parts_ru.append("Да, с такими жалобами к нам обращаются 🌿 Боль в спине, протрузии и грыжи относятся к нашему профилю. Подскажите, пожалуйста, сколько Вам лет?")
        parts_kk.append("Иә, мұндай шағымдармен бізге жүгінеді 🌿 Арқа ауруы, протрузия және грыжа біздің профильге жатады. Жасыңыз нешеде?")
    else:
...
async def handle_message(chat_id: str, phone: str, user_text: str) -> str:
    ...
    session = _load_session_for_identity(chat_id, phone)
    ...
    if current_profile_status == "profile" and not _contra_has_hard_stop(text) and (
        ...
        or _has_generic_lead_prompt(text)
    ):
