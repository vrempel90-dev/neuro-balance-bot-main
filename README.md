...
- **Детерминированные гарантии остаются в Python:** записать можно только слот,
  ...
  цикл.
- **Календарное правило клиники:** суббота — процедурный день без
  консультационной записи, воскресенье — выходной. Поиск консультационных
  слотов пропускает оба дня, а `days_ahead` считает только допустимые дни.
  Причина недоступности передаётся модели отдельно для субботы и воскресенья.
