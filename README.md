...
- **Детерминированные гарантии остаются в Python:** записать можно только слот,
  ...
  цикл.
- **Суббота — процедурный день, воскресенье — выходной:** консультационные
  записи в оба дня не предлагаются; `days_ahead` считается в допустимых
  рабочих днях. Правило живёт в `get_available_slots`, а объяснение пациенту
  пишет модель.
- **Технически недоступный агент** (нет `OPENAI_API_KEY`, выключен `AI_ENABLED`
  или `OPENAI_BRAIN_ENABLED`, исчерпан AI-бюджет, ошибка транспорта) передаёт
  ход администратору; если сбой случился после подтверждённого CRM booking,
  пациент получает детерминированное подтверждение из tool-result. Обещаний
  «уточню и вернусь позже» без реального будущего job нет.
